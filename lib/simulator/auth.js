...
/**
 * Verifies the Privy access token and returns the DID it was minted for —
 * nothing more. No row is read and none is provisioned.
 *
 * This is the whole check a route needs when the answer does not depend on who
 * is asking, only on their being someone: `token-history` reads public market
 * data through our bot token, so it should cost a signature check and not a
 * database round trip per card.
 *
 * @return {Promise<string>} The Privy DID.
 */
export async function requireDid(request) {
...
export async function requireUser(request) {
  ...
  if (existing.length === 0) {
    // The simulator is open to everyone, so every account is provisioned active;
    // status is written explicitly so it does not depend on the column default.
    ...
    const values = getAdminDids().includes(did)
      ...
      : { id: did, email, wallet, status: "active" };
